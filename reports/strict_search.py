...
def search_wechat(keyword, num=15):
    """搜索微信文章"""
    try:
        cmd = [
            'node',
...
def filter_by_accounts_and_date(articles, accounts, days=30):
    """按公众号和时间过滤"""
...
def generate_report_strict(categories, output_file):
    """生成严格符合要求的报告"""
...
def main():
    ...
    search_queries = [
        # 银行业
        "银行业", "数字银行", "零售银行", "银行科技", "信贷管理",
        "商业银行", "银行转型", "银行创新", "银行数字化",
        
        # 金融科技
        "金融科技", "FinTech", "支付创新", "区块链金融", "监管科技",
        "科技金融", "数字货币", "数字人民币", "开放银行",
        
        # AI咨询
        "AI金融", "人工智能银行", "大模型金融", "智能风控", "生成式AI",
        "机器学习金融", "智能客服", "AI咨询", "金融AI",
        
        # 国外金融科技
        "国际金融科技", "海外支付", "跨境金融", "欧洲数字银行", "美国FinTech",
        "新加坡金融科技", "香港金融", "日本支付", "韩国区块链"
    ]
    
    print("开始严格按照要求搜索...")
    print("要求：近30天、全部来自记录的公众号、四大主题、40篇以上")
    ...
    for keyword in search_queries:
        print(f"关键词: {keyword}")
        articles = search_wechat(keyword, num=10)
        
        # 过滤：公众号 + 时间
        filtered = filter_by_accounts_and_date(articles, accounts, days=30)
        ...
        print(f"  找到 {len(filtered)} 篇，去重后新增 {len([a for a in filtered if a.get('title', '') not in seen_titles])} 篇")
        
        # 如果已经达到目标，可以提前停止
        if len(all_articles) >= 50:
            print(f"已达到目标文章数，停止搜索")
            break
    ...
    # 严格分类
    categories = categorize_strict(all_articles)
    ...
    md_content, link_mapping = generate_report_strict(categories, md_file)
    ...
    # 保存原始数据
    data_file = f'strict_search_results_{timestamp}.json'
